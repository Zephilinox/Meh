...
TEST_CASE("Net/WebSocketPacket can write to packet")
{
    ...
    net::PacketHeader<net::MsgClientServer> header = { net::MsgClientServer::Ping, 1000LL };
    net::Packet packet(header);
    CHECK(packet.getHeader().id == net::MsgClientServer::Ping);
    
    //Add some data
    const std::string data = "This is some data\n";
    packet.write(data.data(), data.size());
    CHECK(packet.getHeader().size == sizeof(net::PacketHeader<net::MsgClientServer>) + data.size());
